...
<template>
  <ModalDialog title="Приём оплаты" :subtitle="clientName" width="max-w-2xl" @close="$emit('close')">
        ...
        <div>
          ...
          <div class="flex gap-2">
            <button
              ...
              :class="{ 'border-blue-500 bg-blue-50 text-blue-700': header.currency === c }"
    ...
    <div v-else class="space-y-4">
      ...
      <div>
        <div class="mb-2 text-sm font-semibold text-slate-800">Закрыть долг по накладным</div>
        <div v-if="loadingLines" class="text-sm text-slate-500">Загрузка…</div>
        <div v-else-if="!debtLines.length" class="text-sm text-slate-500">У клиента нет непогашенных накладных.</div>
        <div v-else class="space-y-2">
          <div v-for="line in debtLines" :key="`${line.saleId}:${line.currency}`" class="rounded-lg border border-slate-200 p-3">
            <label class="flex items-center gap-2 text-sm">
              <input
                ...
                @change="toggle(line)"
              />
              <span class="font-medium text-slate-800">{{ line.saleNumber }}</span>
              <span class="text-slate-500">осталось {{ money(line.remaining, line.currency) }}</span>
              <span v-if="selected[`${line.saleId}:${line.currency}`]?.created" class="badge ml-auto bg-emerald-100 text-emerald-700">добавлено</span>
            </label>
      ...
      <p v-if="overAllocated" class="rounded-lg bg-red-50 px-3 py-2 text-xs text-red-600">
