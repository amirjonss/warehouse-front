...
<template>
  <div class="mx-auto max-w-3xl space-y-4">
    ...
    <!-- Шапка документа: доступна и редактируема сразу -->
    <div class="card-pad space-y-3">
      <div class="flex items-center justify-between">
        <div class="text-sm font-semibold text-slate-800 dark:text-slate-100">Новая отгрузка</div>
        <span v-if="draft" class="badge bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400">{{ draft.number }} · черновик</span>
      </div>
      <div>
      ...
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <div>
          ...
          <input v-model="header.note" class="input" @change="syncHeader" />
        </div>
      </div>
    </div>

    <!-- Добавление позиций: на той же странице -->
    <div class="card-pad space-y-3">
      ...
      <button class="btn-ghost w-full" :disabled="!canAddLine || addingItem" @click="addItem">
        <AppIcon name="plus" :size="16" /> Добавить в отгрузку
      </button>
      <p v-if="!header.customerId" class="text-xs text-slate-400 dark:text-slate-500">
